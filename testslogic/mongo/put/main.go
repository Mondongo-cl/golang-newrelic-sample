...
func handler(ctx context.Context, request events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	idValue := request.RequestContext.RequestID
	result, err := coll.InsertOne(ctx, bson.D{{"id", idValue}, {"data", request.Body}})
	if err != nil {
	...
	return events.APIGatewayV2HTTPResponse{
		StatusCode:      200,
		Body:            fmt.Sprintf("{ \"code\":\"200\", \"result\":\"%v\", \"requestid\":\"%v\"}", result.InsertedID, idValue),
		IsBase64Encoded: false,
		Headers:         request.Headers,
	}, nil
}
